...
data "talos_machine_configuration" "machineconfig_cp" {
  ...
  config_patches = [
    yamlencode({
      machine = {
        certSANs = [
          var.talos_cluster_config.name,
          "${var.talos_cluster_config.name}.${var.talos_cluster_config.domain}"
        ]
        network = {
          extraHostEntries = [
            {
              ip = var.talos_cluster_config.proxy.ip
              aliases = var.talos_cluster_config.proxy.aliases
            }
          ]
        }
      }
...
resource "talos_cluster_kubeconfig" "kubeconfig" {
  depends_on           = [ talos_machine_bootstrap.bootstrap, data.talos_cluster_health.health ] # depends_on           = [ talos_machine_bootstrap.bootstrap, data.talos_cluster_health.health ]
  client_configuration = talos_machine_secrets.machine_secrets.client_configuration
  node                 = var.talos_cp_01.ip
}
